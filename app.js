...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showSelectedUnitCard(info) {
    ...
    if (isSold) {
      ...
      else if (isHenleyPark && !isNaN(rawListPrice) && rawListPrice > 0) {
        // 【HENLEY PARK 专属折实价与从价印花税弹窗逻辑】仅对在售/已定价未售且有原价单位生效
        const p0 = rawListPrice;
        const p1 = p0 * (1 - 0.085); // 基础折扣 8.5%

        const sqftArea = parseFloat(String(info.area || '0').replace(/[^0-9\.]/g, ''));
        const rawSqft = parseFloat(String(info.sqftPrice || '').replace(/[^0-9\.]/g, ''));
        const upriceP1 = sqftArea > 0 ? Math.round(p1 / sqftArea) : (rawSqft ? Math.round(rawSqft * 0.915) : 0);
